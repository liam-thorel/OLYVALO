...
assert.equal(compareVersions('4.14.3', '4.14.2'), 1);
assert.equal(compareVersions('v4.14.3', '4.14.3'), 0);
assert.equal(compareVersions('4.14.2', '4.14.3'), -1);
assert.equal(restartDecision(true, '4.14.3'), 'defer');
assert.equal(restartDecision(false, '4.14.3'), 'restart');
assert.equal(restartDecision(false, ''), 'none');
assert.deepEqual(validateManifest({
  version:'4.14.3', files:['index.js','package.json','updater.js','README.md','README.md'],
}, '4.14.3'), ['index.js','package.json','updater.js','README.md']);
assert.throws(() => validateManifest({
  version:'4.14.3', files:['index.js','package.json','updater.js','../secret'],
}, '4.14.3'), /non autorisé/);
assert.throws(() => validateManifest({
...
const releaseFiles = validateManifest(manifest, '4.14.3');
