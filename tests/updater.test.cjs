...
assert.equal(compareVersions('4.13.0', '4.12.0'), 1);
assert.equal(compareVersions('v4.13.0', '4.13.0'), 0);
assert.equal(compareVersions('4.12.0', '4.13.0'), -1);
assert.deepEqual(validateManifest({
  version:'4.13.0', files:['index.js','package.json','updater.js','README.md','README.md'],
}, '4.13.0'), ['index.js','package.json','updater.js','README.md']);
assert.throws(() => validateManifest({
  version:'4.13.0', files:['index.js','package.json','updater.js','../secret'],
}, '4.13.0'), /non autorisé/);
assert.throws(() => validateManifest({
...
const releaseFiles = validateManifest(manifest, '4.13.0');
