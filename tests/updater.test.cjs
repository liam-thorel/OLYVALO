...
assert.equal(compareVersions('4.14.2', '4.14.1'), 1);
assert.equal(compareVersions('v4.14.2', '4.14.2'), 0);
assert.equal(compareVersions('4.14.1', '4.14.2'), -1);
assert.equal(restartDecision(true, '4.14.2'), 'defer');
assert.equal(restartDecision(false, '4.14.2'), 'restart');
assert.equal(restartDecision(false, ''), 'none');
assert.deepEqual(validateManifest({
  version:'4.14.2', files:['index.js','package.json','updater.js','README.md','README.md'],
}, '4.14.2'), ['index.js','package.json','updater.js','README.md']);
assert.throws(() => validateManifest({
  version:'4.14.2', files:['index.js','package.json','updater.js','../secret'],
}, '4.14.2'), /non autorisé/);
assert.throws(() => validateManifest({
...
const releaseFiles = validateManifest(manifest, '4.14.2');
