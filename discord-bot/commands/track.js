...
const { addTracker } = require('../trackers.js');
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('track')
    .setDescription('Suivre les games d\'un membre du roster dans ce salon')
    ...
    .addStringOption(option => option.setName('jeu').setDescription('Jeu à suivre').setRequired(true)
  ...
  async execute(interaction) {
    ...
    await interaction.editReply(created
      ? `✅ Ce salon sera notifié quand **${member.name}** lance une game **${GAME_LABELS[game]}**.`
      : `ℹ️ Ce salon suit déjà **${member.name}** en **${GAME_LABELS[game]}**.`);
  },
};
