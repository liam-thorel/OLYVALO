...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('track-valo-all')
    .setDescription('Suivre les games Valorant de tout le roster OLYCITY dans ce salon')
  ...
  async execute(interaction) {
    ...
    const { created, alreadyTracked, total } = await addTrackerForAll({
      guildId: interaction.guildId, channelId: interaction.channelId, game: 'valorant', addedBy: interaction.user.id,
    }, names);

    await interaction.editReply(
      `✅ Ce salon suit maintenant **Valorant** pour tout le roster (${total} membre(s)).` +
